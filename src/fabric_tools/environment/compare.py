"""Compare Microsoft Fabric Environment definitions."""
...
from fabric_tools.environment.ops import get_environment_definition
...
def compare_environment(client: FabricClient, item: WorkItem) -> CompareResult:
    """Diff a target Environment against a local folder or another remote."""
...
def run_compare_batch(
    ...
    for item in items:
        ...
        progress.advance(status_detail("environment", "comparing", label))
        results.append(compare_environment(client, item))
    return results
...
def _compare_file_to_target(client: FabricClient, item: WorkItem) -> CompareResult:
    ...
    try:
        validate_local_environment(local_path)
    ...
    try:
        remote_text = definition_to_diff_text(
            get_environment_definition(client, target.workspace_id, target.item_id)
        )
...
def _compare_origin_to_target(client: FabricClient, item: WorkItem) -> CompareResult:
    ...
    try:
        origin_text = definition_to_diff_text(
            get_environment_definition(client, origin.workspace_id, origin.item_id)
        )
