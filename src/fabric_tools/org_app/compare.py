"""Compare Microsoft Fabric Org App definitions."""
...
from fabric_tools.org_app.ops import get_org_app_definition
...
def compare_org_app(client: FabricClient, item: WorkItem) -> CompareResult:
    """Diff a target Org App against a local folder or another remote."""
...
def _compare_file_to_target(client: FabricClient, item: WorkItem) -> CompareResult:
    ...
    try:
        validate_local_org_app(local_path)
    ...
    try:
        remote_definition = get_org_app_definition(
...
def _compare_origin_to_target(client: FabricClient, item: WorkItem) -> CompareResult:
    ...
    target_ref = target.label()
    origin_workspace = resolve_workspace_name(client, origin.workspace_id)
    target_workspace = resolve_workspace_name(client, target.workspace_id)
    header = (
        f"origin {local_name} in {origin_workspace}  vs  "
        f"target {remote_name} in {target_workspace}"
    )
    try:
        origin_definition = get_org_app_definition(
            client, origin.workspace_id, origin.item_id
        )
        target_definition = get_org_app_definition(
            client, target.workspace_id, target.item_id
        )
