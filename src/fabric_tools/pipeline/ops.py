...
def run_download_batch(
    ...
    *,
    ...
    for item in items:
        ...
        progress.advance(status_detail("pipeline", "downloading", label))
        results.append(
            download_pipeline(
                client, item, include_schedules=include_schedules, name=label
            )
        )
    return results
    ...
    *,
    ...
    for index, item in enumerate(items):
        ...
        if target is not None and target.is_create:
            progress.advance(status_detail("pipeline", "creating", label))
        elif target is not None:
            progress.advance(status_detail("pipeline", "deploying", label))
        else:
            progress.advance(status_detail("pipeline", "deploying", label))
        results.append(
            deploy_pipeline(
...
def run_delete_batch(client: FabricClient, items: list[WorkItem]) -> list[OpResult]:
    ...
    for item in items:
        label = status_item_label(client, item.target)
        progress.advance(status_detail("pipeline", "deleting", label))
        results.append(delete_pipeline(client, item, name=label))
    return results
