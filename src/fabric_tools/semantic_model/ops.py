...
def run_download_batch(client: FabricClient, items: list[WorkItem]) -> list[OpResult]:
    ...
    for item in items:
        ...
        progress.advance(status_detail("semantic-model", "downloading", label))
        results.append(download_semantic_model(client, item, name=label))
    return results
    ...
    *,
    ...
    for index, item in enumerate(items):
        ...
        if target is not None and target.is_create:
            progress.advance(status_detail("semantic-model", "creating", label))
        elif target is not None:
            progress.advance(status_detail("semantic-model", "deploying", label))
        else:
            progress.advance(status_detail("semantic-model", "deploying", label))
        results.append(
...
def run_delete_batch(client: FabricClient, items: list[WorkItem]) -> list[OpResult]:
    ...
    for item in items:
        label = status_item_label(client, item.target)
        progress.advance(status_detail("semantic-model", "deleting", label))
        results.append(delete_semantic_model(client, item, name=label))
    return results
