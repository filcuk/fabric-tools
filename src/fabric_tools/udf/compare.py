"""Compare remote User Data Function definitions to local folders or other remotes."""
...
from fabric_tools.udf.ops import get_udf_definition
...
def compare_udf(client: FabricClient, item: WorkItem) -> CompareResult:
    """Diff target User Data Function against a local folder or another remote."""
...
def run_compare_batch(
    ...
    for item in items:
        ...
        progress.advance(status_detail("Comparing", "User Data Function", label))
...
def _compare_origin_to_target(client: FabricClient, item: WorkItem) -> CompareResult:
    ...
    try:
        origin_definition = get_udf_definition(
            client, origin.workspace_id, origin.item_id
        )
        target_definition = get_udf_definition(
            client, target.workspace_id, target.item_id
        )
