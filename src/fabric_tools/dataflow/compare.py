"""Compare remote Dataflow Gen2 definitions to local folders or other remotes."""
...
def compare_dataflow(client: FabricClient, item: WorkItem) -> CompareResult:
    """Diff target Dataflow Gen2 against a local folder or another remote."""
...
def _compare_origin_to_target(client: FabricClient, item: WorkItem) -> CompareResult:
    ...
    try:
        origin_definition = get_dataflow_definition(
            client, origin.workspace_id, origin.item_id
        )
        target_definition = get_dataflow_definition(
            client, target.workspace_id, target.item_id
        )
