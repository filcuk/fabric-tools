...
def run_download_batch(client: FabricClient, items: list[WorkItem]) -> list[OpResult]:
    ...
    for item in items:
        ...
        progress.advance(status_detail("variable-library", "downloading", label))
        results.append(download_variable_library(client, item, name=label))
    return results
...
def run_deploy_batch(
    ...
    *,
    ...
    for index, item in enumerate(items):
        ...
        progress.advance(status_detail("variable-library", action, label))
        results.append(
...
def run_delete_batch(client: FabricClient, items: list[WorkItem]) -> list[OpResult]:
    ...
    for item in items:
        label = status_item_label(client, item.target)
        progress.advance(status_detail("variable-library", "deleting", label))
        results.append(delete_variable_library(client, item, name=label))
    return results
