...
- `src/fabric_tools/` — package root
  ...
  - `display.py` — informational GUID / path formatting (`format_guid`, `format_item_ref` → `Name (shortid)`, `format_local_path`); length from `FABRIC_TOOLS_GUID_LENGTH`
...
- **Download:** `-o <workspaceId:artifactId|workspaceId:*>` required; optional `-t <local path>` (defaults to remote display name + kind extension). Local bare stems also get the kind extension (e.g. `myApp` → `myApp.OrgApp`, `ETL` → `ETL.ipynb`); `-t` names the artifact, not a parent folder.
- **Deploy / compare:** one side local path and one side remote (either flag order; roles inferred from type), or remote-to-remote with `-o` source → `-t` destination (`workspaceId:*` expands to overwrites, never create). Local bare stems get the same kind-extension append as download `-t`. Deploy may broadcast one local/remote source to N remotes; compare is 1:1 (unequal post-expand counts fail).
