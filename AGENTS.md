...
- **Deploy / compare:** `-o` local path or remote source; `-t` remote destination (`workspaceId:*` expands to overwrites, never create). Local bare stems on `-o` get the same kind-extension append as download `-t`. Deploy may broadcast one origin to N targets; compare is 1:1 (unequal post-expand counts fail).
