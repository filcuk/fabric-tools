...
- **detail** — optional live progress after ` · ` (e.g. pipeline run activity `3/12 CopyCustomers`); omit when unknown. Keeps the name visible while progress changes.
...
pipeline: running (Load Sales) · 3/12 CopyCustomers…
inspect: listing workspaces…
setup: checking for updates…
```

Build lines with `status_detail(module, action, name=None, detail=None)` (and `progress_message` for `n of m ·` prefixes). Do not put only a name after the module (e.g. `XMLA: Harvest…`); the action is required.
...
**Yes/no prompts:** use `confirm.prompt_confirm` / `confirm.confirm_or_abort` (not raw `typer.confirm`). The prompt text gets the same option / alias / `fabric-tools …` highlighting as panel bodies (`render_cli_prose`, plain when the stream is not a colour terminal). Interactive-wizard questions (questionary) cannot style part of a question, so keep flags and commands out of question text; print them on an `echo_cli_hint` line before the question instead. Decline and Ctrl+C/EOF both become a user abort: `ConfirmationAborted` + Warning panel `Aborted by user.`, never a red `Operation failed.` glued to `[y/N]:`. Sync exits go through `_exit_user_abort` so a stray `typer.Abort` is handled the same way.
